...
var Binder = function () {
  ...
  _createClass(Binder, [{
        ...
        if (importData[from.bindAs]) {
          _lodash2.default.each(importData[from.bindAs], function (toVarName, fromVarName) {
            if (!(fromVarName in from.store)) {
              if (fromVarName === 'balancePayoutAmount') {
                console.log(['from.store', from.store]);
              }
    ...
    value: function importVar(storeName, varName, initiator, raw) {
      ...
      var val = void 0;
      var exportVars = void 0;

      if (s && s.store) {
        exportVars = store.getConfig().exportVars;

        if (exportVars && !exportVars[varName]) {
          console.warn('Warnning! Impossible import variable "' + varName + '" of "' + store.getConfig().bindAs + '" for "' + initiator + '" because variable is not included to config.exportVars.');
          return;
        }

        val = store[varName];
        if (store.getConfig().debug) {
          console.log('Debug "' + storeName + '" import variable "' + varName + '" to "' + initiator + '".', val);
        }
    ...
    value: function callApi(storeName, actionName, initiator) {
      ...
      if (s && s.store) {
        ...
        if (s.store.getConfig().debug) {
          console.log('Debug "' + storeName + '". "' + initiator + '" calls method "' + actionName + '".', arg);
        }
